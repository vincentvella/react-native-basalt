...
`plan/` is gitignored by request -- working notes, milestone plans
(`02-reacthost-surface.md` has the detailed next step), a backlog, and a
decisions log. **It will not come across with a clone.** Either commit it, or
copy it across by hand:

```bash
rsync -a plan/ <mac>:path/to/react-native-linux/plan/
```

Also not in the repo, all regenerated by `bootstrap.sh`: `third_party/`
(folly, fast_float, nlohmann_json, hermes, hermes-build, codegen) and `build/`.
