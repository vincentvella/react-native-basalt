// Drives real ShadowViewMutations through the real MacMountingManager into real
// NSViews, with no JS runtime, no Hermes and no Metro.
//
// The mirror of gtk/mount_harness.cpp, with the same two transactions and the
...
//
// With RN_MAC_SNAPSHOT_DIR set it renders both transactions to PNGs and exits.
...
int runSnapshots(rnlinux::MacMountingManager &manager, const char *directory) {
  ...
  if (!RnMacWriteSnapshot(root, first)) {
    return 1;
  }
  ...
  if (!RnMacWriteSnapshot(root, second)) {
    return 1;
  }
  std::fputs("---\n", stdout);
  std::fputs([[root describeTree] UTF8String], stdout);
...
int main(int argc, const char *argv[]) {
  ...
  (void)argv;
  @autoreleasepool {
    // Constructed on the main thread: MacMountingManager records this thread and
    // asserts every transaction arrives on it.
    rnlinux::MacMountingManager manager;

    if (const char *directory = getenv("RN_MAC_SNAPSHOT_DIR")) {
        ...
        [[NSWindow alloc] initWithContentRect:NSMakeRect(0, 0, 640, 420)
                                    styleMask:NSWindowStyleMaskTitled | NSWindowStyleMaskClosable
                                      backing:NSBackingStoreBuffered
                                        defer:NO];
    window.title = @"react-native-linux — macOS mount harness";
    ...
    rnlinux::MacMountingManager *pending = &manager;
