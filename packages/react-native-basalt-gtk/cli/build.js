 ...
 * build.
 *
 * Two things about where output goes. It goes under the app, in `.basalt`,
 * not into `node_modules/react-native-basalt`, because npm rewrites node_modules
...
const {sharedPackageDir} = require('./shared');

// This package's half: the GTK view layer, the mounting manager and the host.
const NATIVE_DIR = path.resolve(__dirname, '..', 'native');

// The shared half, which is a separate package. CMake finds it on its own in
// both the layouts it can be built from, but `bootstrap.sh` lives there and has
// to be named by path, and pnpm's store is a third layout where CMake's default
// would be wrong -- so the path is passed explicitly rather than relied upon.
const CORE_NATIVE_DIR = path.join(sharedPackageDir(), 'native');
...
function buildHost(context, options) {
  ...
  run(path.join(CORE_NATIVE_DIR, 'bootstrap.sh'), [reactNativeRoot], {
    cwd: workDir,
    env: {...process.env, BASALT_THIRD_PARTY: thirdParty},
  });
  ...
  run('cmake', [
    ...
    // Where the shared half is. CMake has a default that covers a checkout and
    // a hoisted node_modules, and naming it here covers pnpm's store too, where
    // the two packages are not siblings on disk at all.
    `-DBASALT_CORE_DIR=${CORE_NATIVE_DIR}`,
  ]);
...
module.exports = {buildHost, NATIVE_DIR, CORE_NATIVE_DIR};
