...
> **Done, 2026-09-12.** Real `ShadowViewMutation`s mount onto real views. 103
...
`<ScrollView>`, `<TextInput>`, the touch dispatcher and the host. And the image
load is synchronous, which is wrong: a remote image stalls the UI thread. GTK
reads on a worker and decodes back on the main thread, and matching it means the
load has to survive the deletion of the view that asked for it -- a change
rather than a move, and the next thing to do.
