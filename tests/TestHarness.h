...
#if defined(CHECK_EQ) && !defined(RN_LINUX_TEST_HARNESS_ALLOW_GLOG_CHECK)
...
#define TEST(name)                                                                                 \
  static void name();                                                                              \
  static ::rnlinux::testing::Registrar registrar_##name(#name, name);                              \
  static void name()
...
#define EXPECT(condition)                                                                           \
  do {                                                                                             \
    if (!(condition)) {                                                                            \
      ::rnlinux::testing::recordFailure(std::string(__FILE__) + ":" + std::to_string(__LINE__),     \
                                        "expected " #condition);                                   \
    }                                                                                              \
  } while (false)
